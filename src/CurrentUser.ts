...
export class CurrentUser {
    ...
    public register = async (name: string, pass: string): Promise<boolean> =>
        axios
            ...
            .then(() => {
                this.snack('已创建用户 正在登录...');
                this.login(name, pass);
                return true;
            })
            .catch((error: AxiosError) => {
                if (!error || !error.response) {
                    this.snack('无网络连接或服务器不可用.');
                    return false;
                }
                const {data} = error.response;
                this.snack(
                    `注册失败: ${data?.error ?? '未知'}: ${data?.errorDescription ?? ''}`
                );
                return false;
            });
    ...
    public login = async (username: string, password: string) => {
        ...
        const name = (browser && browser.name + ' ' + browser.version) || '未知浏览器';
        axios
            .create()
            .request({
            ...
            .then((resp: AxiosResponse<IClient>) => {
                this.snack(`已为您的会话创建了一个名为'${name}'的客户端`);
    ...
    public tryAuthenticate = async (): Promise<AxiosResponse<IUser>> => {
        ...
        return (
            axios
                ...
                .catch((error: AxiosError) => {
                    if (!error || !error.response) {
                        this.connectionError('无网络连接或服务器不可用.');
